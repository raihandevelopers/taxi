...
export default {
  ...
  methods: {
    ...
    getFullImageUrl(filename) {
      ...
      // Case: it's a File object
      if (filename instanceof File) {
        return URL.createObjectURL(filename);
      }

      // Case: it's already a base64 or blob url
      if (typeof filename === 'string' && (filename.startsWith('blob:') || filename.startsWith('data:'))) {
        return filename;
      }
      
      switch (this.imageType) {
        case 'system-admin':
          return `/storage/uploads/system-admin/logo/${filename}`;
        case 'types':
          return `${filename}`;
        case 'user-profile':
          return `${filename}`;
        ...
        case 'icon':
          return `${filename}`;
        case 'drivers-levelup':
          return `${filename}`;  
        case 'push-notification':
          return `/storage/uploads/push-notification/images/${filename}`; 
        default:
  ...
  watch: {
    initialImageUrl(newVal) {
      // If parent resets the image, also clear local state
      if (!newVal) {
        this.imageFile = null;
      }
    }
    ...
    <div id="image-container">
      <div v-if="!imageFile && !initialImageUrl" :style="{ flex: flexStyle, aspectRatio: aspectRatio }" id="image-slot" @click="triggerFileInput">
        <div class="d-flex flex-column align-items-center">
          ...
          <span><i class="ri-image-add-fill fs-24"></i></span>
        </div>
        <input type="file" id="image-upload" ref="fileInput" @change="handleFileChange" style="display: none;" />
      </div>
       <!-- When an initial image exists, but no new image has been uploaded -->
       <div v-else-if="initialImageUrl && !imageFile" :style="{ flex: flexStyle, aspectRatio: aspectRatio }" class="image-wrapper">
        ...
        <!-- Image Options (Remove and Upload) -->
