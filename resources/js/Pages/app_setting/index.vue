...
export default {
    ...
    setup(props) {
        ...
        const toggleActiveStatus = async (id, status) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        await axios.post(`/app_modules/update-status/${id}`, { id: id, status: status });
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/app_modules/delete/${dataId}`);
                const index = results.value.findIndex(data => data.id === dataId);
                if (index !== -1) {
                    results.value.splice(index, 1);
                }
                modalShow.value = false;
                Swal.fire(t('success'), t('app_modules_deleted_successfully'), 'success');
            } catch (error) {
                ...
                Swal.fire(t('error'), t('failed_to_delete_app_modules'), 'error');
            }
        };
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/app_modules/list`, { params });
                ...
                console.error(t('error_fetching_app_modules'), error);
            }
        };

        const handlePageChanged = async (page) => {
            localStorage.setItem("app_modules/list", page); // Save to localStorage
            fetchDatas(page);
        };

        const editData = async (result) =>  {
            router.get(`/app_modules/edit/${result.id}`); 
        };
    ...
    mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("app_modules/list");
        if(savedPage){
...
<template>
    <Layout>
        ...
        <PageHeader :title="$t('app_modules')" :pageTitle="$t('app_modules')" />
        <BRow>
            <BCol lg="12">
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <Link href="/app_modules/create" v-if="permissions.includes('add_app_modules')">
                                    <BButton variant="primary" class="float-end"> <i
                                            class="ri-add-line align-bottom me-1"></i> {{$t("add_app_modules")}}</BButton>
                                    </Link>
                                </div>
                            </BCol>
                        </BRow>
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                <thead class="table-active">
                                    <tr>
                                        ...
                                        <th scope="col">{{$t("mobile_menu_icon")}}</th>
                                ...
                                <tbody v-if="results.length > 0">
                                    ...
                                    <td> <img :src="result.mobile_menu_icon"  alt="" class="avatar-xs rounded-circle me-2"></td>
                                       <td v-if="permissions.includes('toggle_app_modules')">
                                            <div :class="{
                                                    'form-check': true,
                                                    'form-switch': true,
                                                    'form-switch-lg': true,
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <BButton @click.prevent="editData(result)" v-if="permissions.includes('edit_app_modules')"
                                                ...
                                                <BButton class="btn btn-soft-danger btn-sm m-2" size="sm" v-if="permissions.includes('delete_app_modules') && app_for !== 'demo'"
        ...
        <!-- filter -->
        <BOffcanvas v-model="rightOffcanvas" placement="end" :title="$t('app_modules_filters')" header-class="bg-light"
                ...
                <div class="offcanvas-body">
                    ...
                    <div class="mb-4">
                        <label for="status-select"
                            class="form-label text-muted text-uppercase fw-semibold mb-3">{{$t("service_type")}}</label>
                        <BRow class="g-2">
                            <BCol>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" v-model="filter.service_type" type="radio" id="normal" value="normal"/>
                                    <label class="form-check-label" for="normal">{{$t("normal")}}</label>
                                </div>
                            </BCol>
                            <BCol>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" v-model="filter.rental" type="radio" id="rental" value="rental"/>
                                    <label class="form-check-label" for="rental">{{$t("rental")}}</label>
                                </div>
                            </BCol>
                            <BCol>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input"  v-model="filter.service_type" type="radio" id="outstation" value="outstation"/>
