...
export default {
    ...
    methods: {
    // async downloadInvoice(invoiceType) {
    //   try {
    //     const response = await axios.post(
    //       "/download-invoice",
    //       { invoice_type: invoiceType },
    //       { responseType: "blob" } // Handle PDF file
    //     );
    ...
    // async downloadInvoice(invoiceType) {
    //   try {
    //     const response = await axios.post('/rides-request/download-invoice', {
    //       invoice_type: invoiceType,
    //     }, {
    ...
    //     const link = document.createElement('a');
    //     link.href = url;
    //     link.setAttribute('download', `${invoiceType}_invoice.pdf`); // Set filename dynamically
    //     document.body.appendChild(link);
    //     link.click();
    //     link.remove(); // Cleanup
    //   } catch (error) {
    ...
    // downloadUserInvoice() {
    //   this.downloadInvoice("user");
    // },
    // downloadDriverInvoice() {
    //   this.downloadInvoice("driver");
    // },
    // async downloadInvoice(invoiceType, id) {
    //     try {
    //         const response = await axios.post(
    //             `/rides-request/download-invoice/${id}`,
    //             { invoice_type: invoiceType },
    //             { responseType: 'blob' }
    //         );
    ...
    //         const link = document.createElement('a');
    //         link.href = url;
    //         link.setAttribute('download', `${invoiceType}_invoice_${id}.pdf`);
    //         document.body.appendChild(link);
    //         link.click();
    //         link.remove();
    //     } catch (error) {
    ...
    //     }
    // },
    // downloadUserInvoice(id) {
    //     this.downloadInvoice("user", id);
    // },
    // downloadDriverInvoice(id) {
    //     this.downloadInvoice("driver", id);
    // },
    navigateToInvoice(invoiceType, id) {
        // Navigate to the invoice Blade file
        const url = `/dispatcher/rides_request/download-invoice/${id}?invoice_type=${invoiceType}`;
    ...
    setup(props) {
        ...
        const fetchDatas = async (page = 1) => {
            ...
            try {
                ...
                params.page = page;
                const response = await axios.get(`/dispatcher/rides_request/list`, { params });
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td>{{ result.transport_type=="taxi" ? "Taxi" : "Delivery"}} {{ result.is_bid_ride ? $t('bidding') : '' }}</td> 
                                        <td>
                                            <BBadge :class="{
                                                'text-uppercase': true,
