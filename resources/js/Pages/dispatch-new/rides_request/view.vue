    ...
    setup(props) {
        ...
        try {
            ...
            if(result.value.driverDetail?.data && result.value.is_driver_started){
                database.ref(`drivers/driver_${result.value.driverDetail.data?.id}`).on('value', (snapshot) => {
                                ...
                                <table class="table align-middle position-relative table-nowrap">
                                    ...
                                    <tbody v-else >
                                        ...
                                        <tr v-if="result.additional_charges_amount > 0">
                                            <td> {{result.additional_charges_reason}}</td>
                                            <td>-</td>
                                            <td>{{result.requested_currency_symbol}}{{ result.additional_charges_amount }}</td>
                                        </tr>
                                        <tr v-if="result.airport_surge_fee > 0">
                                            <td> {{$t("airport_surge_fee")}}</td>
                                            <td>-</td>
                                            <td>{{result.requested_currency_symbol}}{{ result.airport_surge_fee }}</td>
                                        </tr>
