...
export default {
    ...
    methods: {
    editData(result) {
      this.$inertia.get(this.route('landing_contact.edit', { id: result.id }));
    },
    ...
    setup(props) {
        ...
        const toggleActiveStatus = async (id, status) => {
            try {
                await axios.post(`/landing-contact/update-status`, { id: id, status: status });
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/landing-contact/delete/${dataId}`);
                const index = results.value.findIndex(data => data.id === dataId);
                if (index !== -1) {
                    results.value.splice(index, 1);
                }
                modalShow.value = false;
                Swal.fire(t('success'), t('landingContact_deleted_successfully'), 'success');
            } catch (error) {
                Swal.fire(t('error'), t('failed_to_delete_landingContact'), 'error');
            }
        };
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/landing-contact/list`, { params });
                results.value = response.data.results;
                paginator.value = response.data.paginator;
                updatePaginatorOptions(paginator.value.total);// Update paginator options dynamically
                modalFilter.value = false;
            } catch (error) {
                console.error(t('error_fetching_contact'), error);
            }
        };

        const handlePageChanged = async (page) => {
          localStorage.setItem("/landing-contact/list", page); // Save to localStorage
            fetchDatas(page);
        };

        const editData = async (result) =>  {
            router.get(`/landing-contact/edit/${result.id}`); 
        };
    ...
    mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("/landing-contact/list");
        if(savedPage){
...
<template>
    <Layout>
        ...
        <Head title="Landing Contact" />
        <PageHeader :title="$t('index')" :pageTitle="$t('landing_contact')" />
        <BRow>
            <BCol lg="12">
                        ...
                        <BRow class="g-2">
                            <BCol class="col-12"md="6">
                                <div class="card" height="100px">
                                            <ImageModal imageSrc="/images/contact.png" imageAlt="contact" width="100px" />
                                    <div class="card-body">
                                        <h4 class="card-title mb-2">{{$t("contactpage")}}</h4>
                                    </div>
                                </div>
                            </BCol>
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <Link href="/landing-contact/create" v-if="permissions.includes('add_landing_contact')">
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item" href="#" @click.prevent="editData(result)" v-if="permissions.includes('edit_landing_contact')">
                                                        <i class="bx bxs-edit-alt align-center text-muted me-2"></i>{{$t("edit")}}
                                                    </a>
                                                    <a class="dropdown-item" href="#" @click.prevent="deleteModal(result.id)" v-if="permissions.includes('delete_landing_contact') && results.length > 1">
