...
export default {
    ...
    setup(props) {
        ...
        const toggleActiveStatus = async (id, status) => {
            try {
                await axios.post(`/landing-home/update-status`, { id: id, status: status });
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/landing-home/delete/${dataId}`);
                const index = results.value.findIndex(data => data.id === dataId);
                if (index !== -1) {
                    results.value.splice(index, 1);
                }
                modalShow.value = false;
                Swal.fire(t('success'), t('landingHome_deleted_successfully'), 'success');
            } catch (error) {
                Swal.fire(t('error'), t('failed_to_delete_landingHome'), 'error');
            }
        };
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/landing-home/list`, { params });
                results.value = response.data.results;
                paginator.value = response.data.paginator;
                updatePaginatorOptions(paginator.value.total);// Update paginator options dynamically
                modalFilter.value = false;
            } catch (error) {
                console.error(t('error_fetching_landingHome'), error);
            }
        };

        const handlePageChanged = async (page) => {
          localStorage.setItem("/landing-home/list", page); // Save to localStorage
            fetchDatas(page);
        };

        const editData = async (result) =>  {
            router.get(`/landing-home/edit/${result.id}`); 
        };
  ...
  mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("/landing-home/list");
        if(savedPage){
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <Link href="/landing-home/create" v-if="permissions.includes('add_landing_home')">
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item" href="#" @click.prevent="editData(result)" v-if="permissions.includes('edit_landing_home')">
                                                        <i class="bx bxs-edit-alt align-center text-muted me-2"></i>{{$t("edit")}}
                                                    </a>
                                                    <a class="dropdown-item" href="#" @click.prevent="deleteModal(result.id)" v-if="permissions.includes('delete_landing_home') && results.length > 1">
