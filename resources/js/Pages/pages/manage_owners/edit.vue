...
export default {
    ...
    setup(props) {
        ...
        const form = useForm({
            password: props.owner ? props.owner.password || "" : "",
            confirm_password: props.owner ? props.owner.password || "" : "",
        });
    ...
    if (Object.keys(errors.value).length === 0) {
        try {
            ...
            let response;
            if (props.owner) {
                response = await axios.post(`/manage-owners/password/update/${props.owner.id}`, formData);
            } else {
...
<template>
    <Layout>
        ...
        <PageHeader :title="manage-owners ? $t('edit') : $t('create')" :pageTitle="$t('owner')" pageLink="/manage-owners" />
        <BRow>
            <BCol lg="12">
