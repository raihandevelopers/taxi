...
export default {
    ...
    setup(props) {
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/title/list`, { params });
        ...
        const handlePageChanged = async (page) => {
          localStorage.setItem("/title/list", page); // Save to localStorage
            fetchDatas(page);
        };
    ...
    mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("/title/list");
        if(savedPage){
...
<template>
    <Layout>
        ...
        <Head title="Title" />
        <PageHeader :title="$t('title')" :pageTitle="$t('support_management')" />
        <BRow>
            <BCol lg="12">
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td>{{ result.user_type }}</td>
                                         <td>{{ result.title_type }}</td>
                                        <td v-if="permissions.includes('toggle-ticket-title')">
                                            <div :class="{
                                                    'form-check': true,
                                                    'form-switch': true,
                                                    'form-switch-lg': true,
                                       ...
                                       <td>
                                        <div class="d-flex">
                                                <BButton @click.prevent="editData(result)" v-if="permissions.includes('edit-ticket-title')"
                                               ...
                                               <BButton class="btn btn-soft-danger btn-sm m-2" size="sm" v-if="permissions.includes('delete-ticket-title')"
