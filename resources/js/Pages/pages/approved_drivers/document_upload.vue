<template>
  <Layout>
    ...
    <PageHeader :title="document ? $t('edit') : $t('upload')" :pageTitle="$t('document_upload')" pageLink="/approved-drivers"/>
    <BRow>
      <BCol lg="12">
                ...
                <div class="row">
                  <div class="col-sm-6">
                    <div class="mb-3">
                      ...
                      <ImageUpload  :imageType="'driver-documents'" :initialImageUrl="form.image"   :flexStyle="'0 0 calc(50% - 20px)'" :aspectRatio="'5 / 5'"   
                  ...
                  <!-- <div v-if="backImageSrc">
                    ...
                    <div class="mb-3">
                      ...
                      <ImageUpload  :imageType="'driver-documents'" :initialImageUrl="form.back_image"   :flexStyle="'0 0 calc(50% - 20px)'" :aspectRatio="'5 / 5'"   
...
export default {
  ...
  setup(props) {
    ...
    const validationRules = {
      name: { required: true },
      image: { required: true },
      back_image: props.document && props.document.image_type === 'front_and_back' ? { required: true } : {},
...
// console.log(props.uploaded);
    ...
    const handleSubmit = async () => {
      ...
      if (form.back_image) {
        formData.append('backImageFile', form.back_image);
      }

      try {
        let response;
        if (props.document && props.document.id) {
          response = await axios.post(`/approved-drivers/document-upload/${props.document.id}/${props.driverId.id}`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          });
        } else {
          response = await axios.post(`/approved-drivers/document-upload/${props.driverId.id}`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          });
        ...
        if (response.status === 201) {
          ...
          form.reset();
          router.get(`/approved-drivers/document/${props.driverId.id}`);
          // if(props.driverId.approve){
          //   router.get(`/approved-drivers/document/${props.driverId.id}`);
          // }else{
