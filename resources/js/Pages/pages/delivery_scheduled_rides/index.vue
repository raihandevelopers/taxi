    ...
    setup(props) {
        ...
        const deleteData = async (dataId) => {
            try {
                ...
                modalShow.value = false;
                Swal.fire(t('success'),t('delivery_scheduled_rides_deleted_successfully') , 'success');
            } catch (error) {
                Swal.fire(t('error'), t('failed_to_delete_delivery_scheduled_rides'), 'error');
            }
        };
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                console.error(t('error_fetching_delivery_scheduled_rides'), error);
            }
        };
...
<template>
    <Layout>
        ...
        <Head title="Delivery Scheduled Rides" />
        <PageHeader :title="$t('delivery_scheduled_ride')" :pageTitle="$t('delivery_scheduled_ride')" />
        <BRow>
            <BCol lg="12">
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                <thead class="table-active">
                                    <tr>
                                        ...
                                        <th scope="col">{{$t("no_of_attempts")}}</th>
                                    ...
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td>
                                            <template v-if="result.active == 1">
                                                <BBadge variant="success" class="text-uppercase">Completed</BBadge>
                                            </template>
                                            <template v-else-if="result.active == 2">
                                                <BBadge variant="success" class="text-uppercase">Cancelled</BBadge>
                                            </template>
                                            <template v-else>
                                                <BBadge variant="danger" class="text-uppercase">Not Started</BBadge>
                                            </template>
                                        </td>  

                                        <td>
                                            <template v-if="result.active == 1">
                                                <BBadge variant="success" class="text-uppercase">Paid</BBadge>
                                            </template>
                                            <template v-else>
                                                <BBadge variant="danger" class="text-uppercase">Not Paid</BBadge>
                                            </template>
                                        </td> 
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">                                                    
                                                    <Link class="dropdown-item" href="/delivery-scheduled-rides/view"><i class=" bx bx-show-alt align-center text-muted me-2"></i> {{$t("view")}}</Link>
                                                </ul>
                                            </div>
                                        </td>
                                     </tr>
