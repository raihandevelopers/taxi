...
export default {
  ...
  const form = useForm({
    ...
    const validationRules = {
      // select_payment_type: { required: true },
  ...
  try {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const response = await axios.post('/report/finance-report-download', form.data(), {
          responseType: 'blob',
        });
        ...
        link.href = url;
        link.setAttribute('download', `finance_report.${form.file_format}`);
        ...
        form.reset();
        router.get('/report/finance-report');
      } catch (error) {
            ...
            <form @submit.prevent="handleSubmit">
              <FormValidation :form="form" :rules="validationRules" ref="validationRef">
                <div class="row">
                  ...
                  <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="trip_status" class="form-label">{{$t("trip_status")}}
                      <!-- <span class="text-danger">*</span> -->
                      </label>
                        <div class="mt-3">
                        <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="inlineRadioOptions"
                        id="WithoutinlineRadio1" :value="0" v-model="form.trip_status">
                        <label class="form-check-label" for="inlineRadio1">{{$t("completed")}}</label>
                        </div>
                        <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="inlineRadioOptions"
                        id="WithoutinlineRadio2"  :value="1" v-model="form.trip_status" checked>
                        <label class="form-check-label" for="inlineRadio1">{{$t("cancelled")}}</label>
                        </div>
                        </div>
                        </div>
                    </div>
                  <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="select_payment_type" class="form-label">{{$t("payment_type")}}</label>
                      <Multiselect 
                        id="select_payment_type" 
                        mode="tags" 
                        v-model="form.select_payment_type" 
                        :close-on-select="false"
                        :searchable="true" 
                        :create-option="true"
                        :options="[
                          { value: '1', label: $t('cash') },
                          { value: '0', label: $t('online') },
                          { value: '2', label: $t('wallet') },
                        ]"
                        :placeholder="$t('select_payment_type')"
                      />
                      <span v-for="(error, index) in errors.select_payment_type" :key="index" class="text-danger">{{ error }}</span>
                    </div>
                  </div>
                   <!-- Date Option Select -->
                  <div class="col-sm-6">
