...
export default {
    ...
    setup(props) {
        ...
        //     try {
        //         await axios.post(`/fleet-needed-documents/toggle`, { id: id, status: status });
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/fleet-needed-documents/delete/${dataId}`);
                const index = results.value.findIndex(data => data.id === dataId);
                if (index !== -1) {
                    results.value.splice(index, 1);
                }
                modalShow.value = false;
                Swal.fire(t('success'), t('fleet_needed_documents_deleted_successfully'), 'success');
            } catch (error) {
                Swal.fire(t('errorr'), t('failed_to_delete_fleet_needed_documents'), 'error');
            }
        };
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/fleet-needed-documents/list`, { params });
                results.value = response.data.results;
                paginator.value = response.data.paginator;
                updatePaginatorOptions(paginator.value.total);// Update paginator options dynamically
                modalFilter.value = false;
            } catch (error) {
                console.error(t('error_fetching_fleet_needed_documents'), error);
            }
        };
        ...
        const editData = async (result) =>  {
            router.get(`/fleet-needed-documents/edit/${result.id}`); 
        };
...
<template>
    <Layout>
        ...
        <PageHeader :title="$t('fleet_needed_documents')" :pageTitle="$t('fleet_needed_documents')" />
        <BRow>
            <BCol lg="12">
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <Link href="/fleet-needed-documents/create" v-if="permissions.includes('add-fleet-driver-document')">
                                    <BButton variant="primary" class="float-end"> <i
                                            class="ri-add-line align-bottom me-1"></i> {{$t("add_fleet_needed_documents")}}</BButton>
                                    </Link>
                                </div>
                            </BCol>
                        </BRow>
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                <thead class="table-active">
                                    <tr>
                                        ...
                                        <th scope="col">{{$t("document_type")}}</th>
                                        <th scope="col">{{$t("has_expiry_date")}}</th>
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td v-if="permissions.includes('toggle-fleet-driver-document')">
                                            <div :class="{
                                                    'form-check': true,
                                                    'form-switch': true,
                                                    'form-switch-lg': true,
                                        ...
                                        <td>
                                            <BButton @click.prevent="editData(result)" v-if="permissions.includes('edit-fleet-driver-document')"
                                            ...
                                            <BButton class="btn btn-soft-danger btn-sm m-2" size="sm" v-if="permissions.includes('delete-fleet-driver-document')"
