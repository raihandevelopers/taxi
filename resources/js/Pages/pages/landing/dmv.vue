  ...
  props: {
    ...
    data() {
        return {
            ...
            landingQuickLinks: {
              dmv: '', // This will hold the HTML content from your database
            },
        };
    },
    created() {
    this.fetchDmvContent();
  },
  methods: {
      fetchDmvContent() {
      const params = new URLSearchParams();
        ...
        axios.get('/api/dmv-content', { params }).then(response => {
            this.landingQuickLinks.dmv = response.data.dmv;
        }).catch(error => {
            console.error("Error fetching dmv content:", error);
        });
      },
      changeLocale(event) {
...
<section class="py-5 mt-5">
    <BContainer>
<PageHeader title="DMV Check" pageTitle="Pages" />
      ...
      <div class="bg-warning-subtle position-relative">
        <BCardBody class="p-5">
          <div class="text-center">
            <h3 style="color: var(--landing_footer_text);">{{ landingQuickLink.dmv_title }}</h3>
          </div>
        </BCardBody>
        <div class="shape">
      ...
      <BCardBody class="p-4">
        <div class="d-flex">
          ...
          <div class="flex-grow-1">
            <div v-html="landingQuickLinks.dmv"></div>
        </div>
    </div>
