  ...
  props: {
    ...
    data() {
        return {
            ...
            landingQuickLinks: {
              compliance: '', // This will hold the HTML content from your database
            },
        };
    },
    created() {
    this.fetchComplianceContent();
  },
  methods: {
    fetchComplianceContent() {
      const params = new URLSearchParams();
        ...
        axios.get('/api/compliance-content', { params }).then(response => {
            this.landingQuickLinks.compliance = response.data.compliance;
        }).catch(error => {
      ...
      <div class="bg-warning-subtle position-relative">
        <BCardBody class="p-5">
          <div class="text-center" >
            <h3 style="color: var(--landing_footer_text);">{{ landingQuickLink.compliance_title }}</h3>
          </div>
        </BCardBody>
        <div class="shape">
      ...
      <BCardBody class="p-4">
        <div class="d-flex">
          ...
          <div class="flex-grow-1">
            <div v-html="landingQuickLinks.compliance"></div>
        </div>
    </div>
