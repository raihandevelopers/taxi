...
class SendWalletAmountMailNotification implements ShouldQueue
{
        ...
        // if(!empty($user?->email)){
            ...
            $notificationData = [
                ...
                'mail_body' => str_replace(['{name}', '{transaction_id}','{currency}', '{amount}', '{current_balance}'],
                              [$this->user->name, $this->transaction_id,$this->currency, $this->amount, $this->user_wallet->amount_balance], 
                ...
                // Send welcome email
                // dd($user->email);
                Mail::to($this->user->email)->send(new WalletAddAmountMail($this->user, $notificationData));
            }
        }
