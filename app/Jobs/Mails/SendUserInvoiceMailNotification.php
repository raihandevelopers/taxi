...
class SendUserInvoiceMailNotification implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($user,$data, $logo, $invoice)
    {
        $this->user = $user;    
        $this->data = $data;
        $this->logo = $logo;
    ...
    public function handle()
    {
        ...
        if ($notification && $notification->mail == 1) {
            ...
            Mail::to($this->user->email)->send(new UserInvoiceMail($this->user, $notificationData, $this->data, $this->logo, $this->invoice ));
        }
